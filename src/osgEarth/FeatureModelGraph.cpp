...
#include <osgEarthCDB/GlobalModelManager>
	...
	if ((size_x > 1) || (size_y > 1))
	{
		...
		for (unsigned int iy = 0; iy < size_y; ++iy)
		{
			for (unsigned int ix = 0; ix < size_x; ++ix)
			{
                ...
                unsigned tileLOD = 0;
                auto load_func = [graph_weakptr,tileLOD, ix, iy, uri, readOptions](Cancelable* c)
                    {
                        osg::ref_ptr<osg::Group> result = nullptr;
                        if (osgEarth::cdbModel::GlobalModelManager::getInstance()->ModelNotLoading(uri))
                        {
                            osg::ref_ptr<FeatureModelGraph> graph;
                            if (graph_weakptr.lock(graph))
                                result = graph->load(0, ix, iy, uri, readOptions.get());
                        }
                        return result;
                    };
                osg::ref_ptr<osg::Node> pagedNode = createPagedNode(
					bs,
					uri,
                    load_func,
					0.0f,
